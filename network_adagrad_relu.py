...
class Network(object):
    def __init__(self, sizes):
        ...
        self.model.add(Dense(units=sizes[1], activation='relu', input_dim=sizes[0]))
        ## Se agrega la capa densa, con sizes[1] neuronas y dimensión sizes[0], f sigmoide
        for size in sizes[2:-1]:
            self.model.add(Dense(units=size, activation='relu'))
        ...
        optimizer = Adagrad(learning_rate=0.0014)
        ## Se setea el optimizador adagrad
        self.model.compile(optimizer=optimizer, loss='mean_squared_error', metrics=['accuracy'])
        ## Se copila el modelo, con los optimizador adagrad, función de perdida mean_squared_error y metrica accuracy
